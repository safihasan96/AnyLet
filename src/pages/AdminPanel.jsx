...
function EnquiryCard({ enquiry, onReply, onResolve, onDelete}) {
 ...
 return (
 <div className="p-6 md:p-8 hover:bg-slate-50/60 :bg-white/[0.02] transition-all border-b border-slate-100 last:border-0">
 {/* Header */}
 ...
 <p className="font-bold text-zinc-950">{enquiry.topic}</p>
 <span className={`text-[9px] font-bold px-2 py-0.5 rounded-md uppercase tracking-widest ${enquiry.type === 'support' ? 'bg-amber-50 text-amber-600' : 'bg-blue-50 text-blue-600'}`}>
 {enquiry.type || 'Support'}
 </span>
 {enquiry.status === 'resolved' ? (
 <span className="text-[9px] font-bold px-2 py-0.5 rounded-md uppercase tracking-widest bg-emerald-50 text-emerald-600">
 ✓ Resolved
 </span>
 ) : (
 <span className="text-[9px] font-bold px-2 py-0.5 rounded-md uppercase tracking-widest bg-red-50 text-red-500">
 Pending
 </span>
 )}
 </div>
 <p className="text-xs font-bold text-slate-400 mt-0.5">{enquiry.userEmail}</p>
 ...
 className="text-[10px] font-bold px-3 py-1.5 rounded-full uppercase tracking-widest bg-emerald-500 hover:bg-emerald-600 text-white transition-colors"
 >
 ✓ Resolve
 </Button>
 )}
 ...
 {/* Conversation thread */}
 <div className="space-y-3 mb-5 ml-4 border-l-2 border-slate-100 pl-5">
 {thread.map((msg, idx) => (
 <div key={idx} className={`${msg.sender === 'admin' ? 'ml-4' : ''}`}>
 <p className={`text-[9px] font-bold uppercase tracking-widest mb-1 ${msg.sender === 'admin' ? 'text-emerald-600' : 'text-zinc-400'}`}>
 ...
 <p className={`text-sm font-medium leading-relaxed p-4 rounded-xl ${msg.sender === 'admin' ? 'bg-emerald-50/70 text-emerald-900 ' : 'bg-slate-50 text-slate-600 '}`}>
 {msg.text}
 </p>
 </div>
 ))}
 ...
 {enquiry.status !== 'resolved' && (
 <div className="ml-4 pl-5 border-l-2 border-dashed border-slate-200 ">
 <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mb-2">Send a Reply</p>
 <form ref={formRef} onSubmit={handleSubmit} className="flex gap-3">
 <Input
 name="reply"
 placeholder="Type your reply here..."
 className="flex-1 bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-400 :border-emerald-500 transition-all"
 />
 <Button
 type="submit"
 className="px-5 py-2.5 bg-slate-900 text-white text-xs font-bold rounded-xl hover:bg-emerald-600 :bg-emerald-600 transition-all whitespace-nowrap"
 >
 Send Reply
 </Button>
 </form>
...
export default function AdminPanel() {
 ...
 confirmText: 'Confirm', confirmColor: 'var(--color-primary)',
 ...
 confirmColor: active ? 'var(--color-danger)' : 'var(--color-primary)',
 onConfirm: async () => {
 ...
 confirmText: 'Delete Forever', confirmColor: 'var(--color-danger)',
 onConfirm: async () => {
 setModal(p => ({ ...p, isLoading: true}));
 try {
 ...
 ═══════════════════════════════════════════════════════════════════════ */
 return (
 <div className="flex h-screen overflow-hidden bg-surface-sunken #0F1117] font-sans">
 ...
 <h1 className="text-sm font-bold text-white leading-none tracking-tight">Architect</h1>
 <div className="flex items-center gap-1.5 mt-1">
 <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse" />
 <p className="text-zinc-500 text-[9px] uppercase font-bold tracking-widest">Live Control</p>
 </div>
 </div>
 </div>
 )}
 ...
 {!isCollapsed && (
 <p className="text-zinc-600 text-[9px] font-bold uppercase tracking-widest px-3 mb-3">Main Core</p>
 )}
 ...
 <div className="w-8 h-8 bg-zinc-800 rounded-xl flex items-center justify-center font-bold text-zinc-300 uppercase text-sm flex-shrink-0">
 {currentUser?.email?.[0] || 'A'}
 </div>
 <div className="overflow-hidden">
 <p className="text-xs font-bold text-white truncate">{currentUser?.email?.split('@')[0]}</p>
 <p className="text-[9px] font-bold text-zinc-500 uppercase tracking-widest">SuperAdmin</p>
 </div>
 </div>
 ...
 <div>
 <div className="flex items-center gap-2 text-zinc-400 text-[9px] font-bold uppercase tracking-widest mb-0.5">
 <span>Platform</span>
 <ChevronRight size={8} />
 <span className="text-emerald-500 capitalize">{activeTab === 'admin' ? 'Dashboard' : activeTab}</span>
 </div>
 <h2 className="text-2xl font-bold text-zinc-950 tracking-tight capitalize">
 ...
 <p className="text-sm font-bold text-zinc-950 leading-none">{currentUser?.email?.split('@')[0]}</p>
 <p className="text-[9px] font-bold text-emerald-500 uppercase tracking-widest mt-0.5">Management</p>
 </div>
 <div className="w-10 h-10 bg-zinc-950 rounded-xl flex items-center justify-center font-bold text-white text-sm">
 ...
 <p className="text-zinc-400 font-bold text-xs uppercase tracking-widest animate-pulse">Syncing data...</p>
 </div>
 ) : (
 ...
 <div className="text-4xl font-bold text-zinc-950 tabular-nums group-hover:text-emerald-600 transition-colors duration-400">{s.value}</div>
 </div>
 ))}
 </div>
 ...
 <span className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest px-2.5 py-1 bg-zinc-50 rounded-lg">Live</span>
 </div>
 <div className="divide-y divide-zinc-50">
 {users.slice(0, 5).map((user, i) => (
 <div key={i} className="flex items-center justify-between px-8 py-4 hover:bg-zinc-50/50 transition-colors">
 <div className="flex items-center gap-4">
 <div className="w-9 h-9 bg-zinc-100 rounded-xl flex items-center justify-center font-bold text-zinc-400 text-xs uppercase">{user.email?.[0]}</div>
 <div>
 ...
 <span className="text-[10px] font-bold text-zinc-400 tracking-widest">LIVE</span>
 </div>
 ))}
 </div>
 </div>
 ...
 <div>
 <p className="text-emerald-400 text-[9px] font-bold uppercase tracking-widest mb-3 flex items-center gap-2">
 <Lock size={10} /> Security Status
 </p>
 <div className="flex items-baseline gap-2">
 <span className="text-4xl font-bold">AES-256</span>
 <span className="text-emerald-500 text-[9px] font-bold animate-pulse uppercase tracking-widest">Active</span>
 </div>
 </div>
 <Button
 onClick={handleSystemCleanup}
 className="w-full py-4 bg-emerald-500 hover:bg-emerald-600 text-white font-bold rounded-2xl transition-all text-[10px] uppercase tracking-[0.2em] shadow-lg shadow-emerald-500/20 active:scale-[0.98] flex items-center justify-center gap-2"
 >
 ...
 <div>
 <h3 className="text-2xl font-bold text-zinc-950">Platform Directory</h3>
 ...
 <thead>
 <tr className="text-zinc-400 text-[10px] font-bold uppercase tracking-widest border-b border-zinc-50">
 ...
 <div className="w-12 h-12 bg-zinc-100 rounded-2xl flex items-center justify-center font-bold text-zinc-300 text-lg uppercase group-hover:border-emerald-500 group-hover:text-emerald-500 border border-transparent transition-all">
 {user.email?.[0]}
 </div>
 <div>
 <p className="font-bold text-zinc-950 tracking-tight group-hover:text-emerald-600 transition-colors">{user.fullName || 'Anonymous'}</p>
 ...
 <span className={`text-[10px] font-bold px-3 py-1 rounded-full uppercase tracking-widest ${user.accountStatus === 'deactivated' ? 'bg-red-50 text-red-600' : 'bg-emerald-50 text-emerald-600'}`}>
 {user.accountStatus === 'deactivated' ? 'Inactive' : 'Active'}
 </span>
 <span className="text-[9px] font-bold text-zinc-400 uppercase tracking-widest">{user.role || 'client'}</span>
 </div>
 </td>
 <td className="px-8 py-6">
 ...
 <div className="px-8 py-8 flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 border-b border-zinc-50">
 <div>
 <h3 className="text-2xl font-bold text-zinc-950">Property Approvals</h3>
 ...
 <Button onClick={() => setPropertiesTab('pending')} className={`flex-1 sm:flex-none px-4 py-2 rounded-lg text-xs font-bold transition-all ${propertiesTab === 'pending' ? 'bg-white shadow-sm text-amber-500' : 'text-zinc-400 hover:text-zinc-600'}`}>Pending ({pendingListings})</Button>
 <Button onClick={() => setPropertiesTab('approved')} className={`flex-1 sm:flex-none px-4 py-2 rounded-lg text-xs font-bold transition-all ${propertiesTab === 'approved' ? 'bg-white shadow-sm text-emerald-500' : 'text-zinc-400 hover:text-zinc-600'}`}>Approved</Button>
 <Button onClick={() => setPropertiesTab('rejected')} className={`flex-1 sm:flex-none px-4 py-2 rounded-lg text-xs font-bold transition-all ${propertiesTab === 'rejected' ? 'bg-white shadow-sm text-rose-500' : 'text-zinc-400 hover:text-zinc-600'}`}>Rejected</Button>
 <Button onClick={() => setPropertiesTab('all')} className={`flex-1 sm:flex-none px-4 py-2 rounded-lg text-xs font-bold transition-all ${propertiesTab === 'all' ? 'bg-white shadow-sm text-zinc-900' : 'text-zinc-400 hover:text-zinc-600'}`}>All</Button>
 </div>
 ...
 <p className="font-bold text-zinc-950 group-hover:text-emerald-600 transition-colors">
 {listing.title || 'Untitled Listing'}
 </p>
 <span className="text-[10px] font-mono font-bold text-zinc-400 bg-zinc-100 px-1.5 py-0.5 rounded uppercase">
 ...
 {listing.rent && (
 <p className="text-xs font-bold text-emerald-600 mt-0.5">৳{Number(listing.rent).toLocaleString()}/mo</p>
 )}
 </div>
 </div>
 </td>
 ...
 {listing.isApproved ? (
 <span className="inline-flex items-center gap-1.5 text-[10px] font-bold px-3 py-1.5 rounded-full bg-emerald-50 text-emerald-600 uppercase tracking-widest">
 <CheckCircle size={11} /> Verified
 </span>
 ) : listing.isRejected ? (
 <span className="inline-flex items-center gap-1.5 text-[10px] font-bold px-3 py-1.5 rounded-full bg-rose-50 text-rose-600 uppercase tracking-widest">
 <X size={11} /> Rejected
 </span>
 ) : (
 <span className="inline-flex items-center gap-1.5 text-[10px] font-bold px-3 py-1.5 rounded-full bg-amber-50 text-amber-600 uppercase tracking-widest">
 ...
 className="px-3 py-1.5 bg-emerald-500 hover:bg-emerald-600 text-white text-xs font-bold rounded-xl transition-all shadow-sm active:scale-95 flex items-center gap-1"
 >
 Approve
 </Button>
 {!listing.isRejected && (
 ...
 className="px-3 py-1.5 bg-rose-500 hover:bg-rose-600 text-white text-xs font-bold rounded-xl transition-all shadow-sm active:scale-95 flex items-center gap-1"
 >
 Reject
 </Button>
 )}
 ...
 className="px-6 py-2.5 bg-white border border-zinc-200 text-zinc-600 font-bold text-xs uppercase tracking-widest rounded-xl shadow-sm hover:bg-zinc-50 transition-all active:scale-95"
 >
 Load More
 </Button>
 </div>
 ...
 <h3 className="text-2xl font-bold text-zinc-950">Support Enquiries</h3>
 <p className="text-sm text-zinc-400 font-bold mt-1">
 ...
 <div className="bg-white rounded-3xl border border-zinc-100 shadow-sm overflow-hidden">
 <div className="px-8 py-8 border-b border-zinc-50">
 <h3 className="text-2xl font-bold text-zinc-950">Payment Verification</h3>
 <p className="text-sm text-zinc-400 font-bold mt-1">
 ...
 <td className="px-8 py-6">
 <p className="font-bold text-zinc-950 tracking-tight">{(payment.type || '').replace('_', ' ').toUpperCase()}</p>
 {payment.type === 'subscription' && payment.metadata?.plan ? (
 <p className="text-[10px] font-bold text-emerald-600 uppercase tracking-widest mt-1">Package: {payment.metadata.plan}</p>
 ) : (
 <p className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest mt-1">Prop: {payment.propertyName || 'N/A'}</p>
 )}
 </td>
 <td className="px-8 py-6">
 ...
 <td className="px-8 py-6 text-center">
 <span className={`text-[10px] font-bold px-3 py-1 rounded-full uppercase tracking-widest ${payment.status === 'completed' ? 'bg-emerald-50 text-emerald-600' : payment.status === 'failed' ? 'bg-red-50 text-red-600' : 'bg-amber-50 text-amber-600'}`}>
 {payment.status}
 </span>
 </td>
 <td className="px-8 py-6">
 ...
 <div className="bg-white rounded-3xl border border-zinc-100 shadow-sm overflow-hidden">
 <div className="px-8 py-8 border-b border-zinc-50">
 <h3 className="text-2xl font-bold text-zinc-950">Escrow Management</h3>
 <p className="text-sm text-zinc-400 font-bold mt-1">
 ...
 <td className="px-8 py-6">
 <p className="font-bold text-zinc-950 tracking-tight">{escrow.propertyName}</p>
 <p className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest mt-1">Tenant ID: {escrow.tenantId?.slice(0,6)}...</p>
 </td>
 <td className="px-8 py-6">
 <span className="font-bold text-emerald-600">৳{escrow.depositAmount?.toLocaleString()}</span>
 </td>
 <td className="px-8 py-6 text-center">
 <span className={`text-[10px] font-bold px-3 py-1 rounded-full uppercase tracking-widest ${escrow.status === 'released' ? 'bg-emerald-50 text-emerald-600' : 'bg-blue-50 text-blue-600 '}`}>
 {escrow.status}
 </span>
 </td>
 <td className="px-8 py-6 text-center">
 <div className="flex flex-col gap-1 items-center">
 <span className={`text-[9px] font-bold uppercase tracking-widest ${escrow.confirmedByTenant ? 'text-emerald-500' : 'text-zinc-400'}`}>
 Tenant: {escrow.confirmedByTenant ? 'Yes' : 'No'}
 </span>
 <span className={`text-[9px] font-bold uppercase tracking-widest ${escrow.confirmedByOwner ? 'text-emerald-500' : 'text-zinc-400'}`}>
 ...
 <Button onClick={() => handleReleaseEscrow(escrow.id)} className="px-4 py-2 bg-emerald-500 text-white font-bold text-[10px] uppercase tracking-widest rounded-xl hover:bg-emerald-600 transition-colors shadow-lg shadow-emerald-500/20">
 Release Funds
 </Button>
 </div>
 )}
 ...
 <div className="bg-white rounded-3xl border border-zinc-100 shadow-sm overflow-hidden">
 <div className="px-8 py-8 border-b border-zinc-50">
 <h3 className="text-2xl font-bold text-zinc-950">Webhook SMS Transactions</h3>
 <p className="text-sm text-zinc-400 font-bold mt-1">
 ...
 <td className="px-8 py-6 text-center">
 <span className={`text-[10px] font-bold px-3 py-1 rounded-full uppercase tracking-widest ${txn.status === 'claimed' ? 'bg-emerald-50 text-emerald-600' : 'bg-amber-50 text-amber-600'}`}>
 {txn.status}
 </span>
 </td>
 <td className="px-8 py-6">
 ...
 <div className="flex justify-center">
 <Button onClick={() => handleApproveWebhookTxn(txn)} className="px-4 py-2 bg-emerald-50 text-emerald-600 font-bold text-[10px] uppercase tracking-widest rounded-xl hover:bg-emerald-500 hover:text-white transition-colors">
 Approve
 </Button>
 </div>
 )}
 ...
 <div>
 <h3 className="text-2xl font-bold text-zinc-950">SMS Webhook Payment Details</h3>
 <p className="text-sm text-zinc-400 font-bold mt-1">
 ...
 <span className="text-[10px] font-bold uppercase tracking-widest text-emerald-600">{webhookTxns.length > 0 ? 'Live' : 'Waiting'}</span>
 </div>
 </div>
 </div>
 <div className="overflow-x-auto">
 ...
 {txn.bookingType && (
 <span className="text-[9px] font-bold text-blue-500 uppercase tracking-widest">{txn.bookingType}</span>
 )}
 </div>
 ) : (
 ...
 className="px-3 py-2 bg-amber-50 text-amber-600 font-bold text-[10px] uppercase tracking-widest rounded-xl hover:bg-amber-500 hover:text-white transition-colors shadow-sm"
 ...
 <div>
 <h2 className="text-2xl font-bold text-zinc-950 tracking-tight">Review Moderation</h2>
 ...
 <div>
 <h2 className="text-2xl font-bold text-zinc-950 tracking-tight">KYC Verification</h2>
 ...
 <div className="bg-white rounded-3xl border border-zinc-100 shadow-sm overflow-hidden">
 <div className="px-8 py-8 border-b border-zinc-50">
 <h3 className="text-2xl font-bold text-zinc-950">Property Reports</h3>
 <p className="text-sm text-zinc-400 font-bold mt-1">
 ...
 <div>
 <p className="text-[10px] font-bold text-rose-500 uppercase tracking-widest mb-1">Reason: {report.reason}</p>
 ...
 <div className="flex items-center gap-2">
 <div className="w-6 h-6 bg-zinc-200 rounded-lg flex items-center justify-center text-[10px] font-bold text-zinc-600 uppercase">
 ...
 className="flex-1 py-3 px-4 bg-zinc-950 text-white text-[10px] font-bold uppercase tracking-widest rounded-xl hover:bg-zinc-800 transition-all flex items-center justify-center gap-2"
 >
 <Search size={14} /> View Ad
 </Button>
 <Button 
 onClick={() => handleDeleteReportedProperty(report)}
 className="flex-1 py-3 px-4 bg-rose-500 text-white text-[10px] font-bold uppercase tracking-widest rounded-xl hover:bg-rose-600 transition-all shadow-lg shadow-rose-500/20 flex items-center justify-center gap-2"
 >
 <Trash2 size={14} /> Delete Ad
 </Button>
 <Button 
 onClick={() => handleDismissReport(report.id)}
 className="flex-1 py-3 px-4 bg-zinc-100 text-zinc-500 text-[10px] font-bold uppercase tracking-widest rounded-xl hover:bg-zinc-200 transition-all flex items-center justify-center gap-2"
 >
 ...
 <p className="text-xl font-bold text-zinc-950 mb-2 uppercase tracking-wide">Module Operational</p>
 <p className="text-sm font-bold text-zinc-400 max-w-sm">This module is live and monitoring connections within the cluster.</p>
 </div>
 <Link to="/admin"
 className="px-8 py-3 bg-zinc-950 text-white rounded-2xl font-bold text-[10px] uppercase tracking-widest hover:bg-emerald-600 transition-all duration-300">
 ...
 <div>
 <p className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest mb-1">Listing Detail</p>
 <h3 className="text-xl font-bold text-zinc-950 leading-tight">{selectedListing.title || 'Untitled Listing'}</h3>
 </div>
 <div className="flex items-center gap-3">
 ...
 {/* Location grid */}
 <div>
 <p className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest mb-3">Location</p>
 ...
 {/* Rent & Details */}
 <div>
 <p className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest mb-3">Pricing & Details</p>
 <div className="grid grid-cols-2 gap-3">
 {[
 ...
 <div>
 <p className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest mb-3">Features</p>
 ...
 <p className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest mb-3">Owner / Landlord</p>
 {ownerLoading ? (
 ...
 <div className="w-11 h-11 bg-zinc-200 rounded-xl flex items-center justify-center font-bold text-zinc-500 text-sm uppercase flex-shrink-0">
 {listingOwner.email?.[0] || '?'}
 </div>
 <div className="space-y-0.5">
 <p className="font-bold text-zinc-950">{listingOwner.fullName || listingOwner.name || 'No name'}</p>
 ...
 <p className="text-[9px] font-bold text-zinc-400 uppercase tracking-widest pt-1">{listingOwner.role || 'user'} · ID: {listingOwner.id?.slice(0, 10)}...</p>
 </div>
 </div>
 ) : (
 ...
 className={`w-full py-3.5 font-bold rounded-2xl transition-all text-sm active:scale-[0.98] flex items-center justify-center gap-2 ${selectedListing.isVerified ? 'bg-amber-50 text-amber-600 hover:bg-amber-100' : 'bg-emerald-50 text-emerald-600 hover:bg-emerald-100 border border-emerald-100'}`}
 >
 <ShieldCheck size={18} />
 ...
 className="flex-1 py-3.5 bg-emerald-500 hover:bg-emerald-600 text-white font-bold rounded-2xl transition-all text-sm shadow-lg shadow-emerald-500/20 active:scale-[0.98]"
 >
 ✓ Approve
 </Button>
 ...
 className="flex-1 py-3.5 bg-rose-500 hover:bg-rose-600 text-white font-bold rounded-2xl transition-all text-sm shadow-lg shadow-rose-500/20 active:scale-[0.98]"
 >
 ✕ Reject
 </Button>
 )}
 ...
 {/* Modal Card */}
 <div
 className="bg-white rounded-[32px] w-full max-w-lg overflow-hidden shadow-2xl border border-zinc-100 "
 onClick={e => e.stopPropagation()}
 >
 {/* Header */}
 <div className="px-8 py-6 border-b border-zinc-100 flex items-center justify-between">
 <div>
 <p className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest mb-1">User Profile Card</p>
 <h3 className="text-xl font-bold text-zinc-950 leading-tight">Identity Details</h3>
 </div>
 <Button
 onClick={() => setSelectedUser(null)}
 className="w-9 h-9 rounded-xl bg-zinc-50 flex items-center justify-center text-zinc-400 hover:text-zinc-900 :text-white transition-all"
 >
 <X size={18} />
 </Button>
 </div>
 ...
 {/* Profile Head */}
 <div className="flex items-center gap-5 bg-zinc-50 p-6 rounded-3xl">
 {selectedUser.photoURL ? (
 <img loading="lazy" src={selectedUser.photoURL} alt="Profile" className="w-16 h-16 rounded-2xl object-cover" />
 ) : (
 <div className="w-16 h-16 bg-emerald-500 rounded-2xl flex items-center justify-center font-bold text-white text-2xl uppercase">
 {selectedUser.fullName?.[0] || selectedUser.email?.[0] || '?'}
 </div>
 )}
 <div>
 <h4 className="text-lg font-bold text-zinc-950 ">{selectedUser.fullName || 'Anonymous User'}</h4>
 <p className="text-sm font-bold text-zinc-500 ">{selectedUser.email}</p>
 <span className="inline-block mt-2 text-[9px] font-bold px-2.5 py-1 bg-emerald-100 text-emerald-800 rounded-full uppercase tracking-widest">
 ...
 <p className="text-[9px] font-bold text-zinc-400 uppercase tracking-widest">Account Status</p>
 <p className="text-sm font-bold text-zinc-950 mt-1 capitalize">{selectedUser.accountStatus || 'Active'}</p>
 </div>
 <div className="bg-zinc-50 p-4 rounded-2xl">
 <p className="text-[9px] font-bold text-zinc-400 uppercase tracking-widest">Contact Phone</p>
 <p className="text-sm font-bold text-zinc-950 mt-1">{selectedUser.phone || selectedUser.contact || 'Not provided'}</p>
 </div>
 <div className="bg-zinc-50 p-4 rounded-2xl">
 <p className="text-[9px] font-bold text-zinc-400 uppercase tracking-widest">Subscription Plan</p>
 <p className="text-sm font-bold text-zinc-950 mt-1">{selectedUser.subscriptionPlan || 'Free'}</p>
 </div>
 <div className="bg-zinc-50 p-4 rounded-2xl">
 <p className="text-[9px] font-bold text-zinc-400 uppercase tracking-widest">KYC Status</p>
 ...
 <div className="border-t border-zinc-100 pt-6">
 <p className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest mb-3">Government Issued ID</p>
 <a
 href={selectedUser.verification.idDocumentUrl}
 target="_blank"
 rel="noopener noreferrer"
 className="block overflow-hidden rounded-2xl border border-zinc-200 hover:opacity-90 transition-opacity"
 >
 ...
 <Button
 onClick={() => setSelectedUser(null)}
 className="px-6 py-2.5 bg-zinc-950 hover:bg-zinc-800 text-white font-bold rounded-xl text-sm transition-all"
 >
 Close Details
 </Button>
 </div>
